...
function cloudflare_single_img( $image )
{
  ...
  $newurl = $url['scheme'] . '://' . $url['host'] . '/cdn-cgi/image/width=' . $image[1] . ',height=' . $image[2] . ',quality=81,format=auto,onerror=redirect,metadata=none' . $url['path'];
...
function cloudflare_srcset($sources)
{
  ...
  foreach ($sources as $key => $value)
  {
    ...
    $cfparams .= ',quality=82,format=auto,onerror=redirect,metadata=none';
...
function cloudflare_get_attachment_url($url, $post_id)
{
  ...
  if(!strstr($url, '/cdn-cgi/image/'))
  {
    ...
    if(in_array(strtolower(pathinfo($safe_text, PATHINFO_EXTENSION)), [ 'jpg', 'jpeg', 'gif', 'png', 'webp' ]))
    {
      ...
      $newurl = $old_url['scheme'] . '://' . $old_url['host'] . '/cdn-cgi/image/quality=83,format=auto,onerror=redirect,metadata=none' . $old_url['path'];
  ...
  if(filter_var($safe_text, FILTER_VALIDATE_URL))
  {
    ...
    if(!strstr($safe_text, '/cdn-cgi/image/'))
    {
      ...
      if(in_array(strtolower(pathinfo($safe_text, PATHINFO_EXTENSION)), [ 'jpg', 'jpeg', 'gif', 'png', 'webp' ]))
      {
        ...
        $newurl = $old_url['scheme'] . '://' . $old_url['host'] . '/cdn-cgi/image/quality=84,format=auto,onerror=redirect,metadata=none' . $old_url['path'];
...
function cloudflare_clean_url($url, $protocols, $context)
{
  ...
  if(in_array(strtolower(pathinfo($url, PATHINFO_EXTENSION)), [ 'jpg', 'jpeg', 'gif', 'png', 'webp' ]))
  {
    if(!strstr($url, '/cdn-cgi/image/'))
    {
      ...
      $newurl = $old_url['scheme'] . '://' . $old_url['host'] . '/cdn-cgi/image/quality=85,format=auto,onerror=redirect,metadata=none' . $old_url['path'];
